...
function groupRows(rows = [], groups = [], totals = {}) {
    if (!rows) {
        return [];
    }
...
class KetchupDataTable {
    ...
    renderCell(cell) {
        ...
        if (isIcon(cell.obj) || isVoCodver(cell.obj)) {
            ...
            content = h("kup-graphic-cell", { value: cell.value });
        }
...
class KetchupDataTable$1 {
    ...
    drawGraphicCell() {
        ...
        vGraphicElementDefinitionArr.forEach((graphicElem, index) => {
            ...
            for (const sep of vSeparatorsList) {
    ...
    getNewStarXFromTril(startX, tril) {
        const newStartX = this.getDim(this.canvas.width, tril.getWidth());
    ...
    getNewStarXFromTrir(startX, trir) {
        const newStartX = this.getDim(this.canvas.width, trir.getWidth());
...
export { KetchupDataTable as KupDataTable, KetchupDataTable$1 as KupGraphicCell, KetchupPaginator as KupPaginator, KetchupTextInput as KupTextInput };
