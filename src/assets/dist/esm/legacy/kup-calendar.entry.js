...
var KupCalendar = /** @class */ (function () {
    function KupCalendar(hostRef) {
        registerInstance(this, hostRef);
        this.weekView = false;
        this.hideNavigation = false;
        this.calendarContainer = null;
        this.kupCalendarEventClicked = createEvent(this, "kupCalendarEventClicked", 6);
        this.kupCalendarDateClicked = createEvent(this, "kupCalendarDateClicked", 6);
        this.kupCalendarEventDropped = createEvent(this, "kupCalendarEventDropped", 6);
        this.kupCalendarViewChanged = createEvent(this, "kupCalendarViewChanged", 6);
    }
    // ---- Private methods ----
    KupCalendar.prototype.getColumns = function () {
        if (this.data && this.data.rows) {
            return this.data.columns;
        }
        return [];
    };
    KupCalendar.prototype.getRows = function () {
        if (this.data && this.data.rows) {
            return this.data.rows;
        }
        return [];
    };
    KupCalendar.prototype.getEvents = function () {
        var _this_1 = this;
        var isHourRange = this.startCol &&
            this.endCol &&
        ...
        return this.getRows().map(function (row) {
            var startDate = formatToMomentDate(row.cells[_this_1.dateCol]);
            var endDate = formatToMomentDate(row.cells[_this_1.dateCol]);
            if (isHourRange) {
                var startCell = row.cells[_this_1.startCol];
                var endCell = row.cells[_this_1.endCol];
                if (startCell && endCell) {
                    var momentStart = moment(startCell.value, 'HH:mm:ss');
                    var momentEnd = moment(endCell.value, 'HH:mm:ss');
            ...
            var allDay = !isHourRange;
            return {
                title: row.cells[_this_1.descrCol].value,
                allDay: allDay,
    ...
    KupCalendar.prototype.onPrev = function () {
        this.calendar.prev();
        this.emitNavEvent();
    };
    KupCalendar.prototype.onNext = function () {
        this.calendar.next();
        this.emitNavEvent();
    };
    KupCalendar.prototype.emitNavEvent = function () {
        // see https://fullcalendar.io/docs/view-object
        var to = moment(this.calendar.view.currentEnd)
            .subtract(1, 'day')
            .toDate();
    ...
    KupCalendar.prototype.onToday = function () {
        this.calendar.today();
    };
    // ---- Lifecycle ----
    KupCalendar.prototype.componentDidLoad = function () {
        var _this_1 = this;
        var plugins = [main$2];
        if (this.weekView) {
            plugins.push(main$1);
        ...
        this.calendar = new Calendar(this.calendarContainer, {
            ...
            eventRender: function (info) {
                if (_this_1.styleCol) {
                    ...
                    var cell_1 = row.cells[_this_1.styleCol];
                    if (cell_1 && cell_1.style) {
                        Object.keys(cell_1.style).forEach(function (k) { return (info.el.style[k] = cell_1.style[k]); });
                    }
                }
                if (_this_1.iconCol) {
                    var row = info.event.extendedProps.row;
                    var cell = row.cells[_this_1.iconCol];
                    if (cell && cell.value) {
                        var wrapper_1 = document.createElement('div');
                        wrapper_1.classList.add('icon-wrapper');
                        cell.value.split(';').forEach(function (icon) {
                            var span = document.createElement('span');
                            span.className = icon;
                            wrapper_1.appendChild(span);
                        });
                        info.el.appendChild(wrapper_1);
                    }
                }
                if (_this_1.imageCol) {
                    var row = info.event.extendedProps.row;
                    var cell = row.cells[_this_1.imageCol];
                    if (cell && cell.value) {
                        var wrapper_2 = document.createElement('div');
                        wrapper_2.classList.add('image-wrapper');
                        cell.value.split(';').forEach(function (icon) {
                            var img = document.createElement('img');
                            img.src = icon;
                            wrapper_2.appendChild(img);
                        });
                        info.el.appendChild(wrapper_2);
                    }
                }
            },
            eventClick: function (_d) {
                var event = _d.event;
                // see https://fullcalendar.io/docs/eventClick
                _this_1.kupCalendarEventClicked.emit(event.extendedProps.row);
            },
            eventDrop: function (_d) {
                var event = _d.event, oldEvent = _d.oldEvent;
                // https://fullcalendar.io/docs/eventDrop
                _this_1.kupCalendarEventDropped.emit({
                    fromDate: {
