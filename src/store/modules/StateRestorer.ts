...
export default class StateRestorer extends VuexModule {
  ...
  @Mutation
  POP_HISTORY() {
    this.currentlyRetrievedState = this.historyArray.pop() || HistoryRecordFactory();
  }
  ...
  @Mutation
  STORE_COMPONENT_STATE({key , toStore}: {key: string, toStore: ComponentStoredState}) {
  ...
  get getComponentState() {
    return (key: string): ComponentStoredState | undefined => {return this.currentHistoryRecord.states[key]};
  }
